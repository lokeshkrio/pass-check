...
from passguard.analysis.dictionary.provider import DictionaryProvider, BuiltinDictionaryProvider


class DictionaryAnalyzer:
    def __init__(self, provider: DictionaryProvider | None = None) -> None:
        self._provider = provider if provider is not None else BuiltinDictionaryProvider()
    ...
    def analyze(self, context: AnalysisContext) -> None:
        ...
        for pwd in passwords_to_check:
            ...
            for i in range(length):
                for j in range(i + 3, length + 1): # only check substrings of length >= 3
